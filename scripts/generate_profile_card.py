...
Requires:
  ...
  - GH_USERNAME   : GitHub username to profile (defaults to "vvinayakkk")
...
USERNAME = os.environ.get("GH_USERNAME", "vvinayakkk")
...
def autocrop_to_subject(img, bust_fraction=1.0):
    ...
    A plain headshot (solid background, no alpha) has no subject bbox to
    find, so fall back to a plain top-fraction crop assuming a centered
    portrait — still spends more of the ASCII grid on the face than the
    uncropped shoulders/torso would."""
    if img.mode == "RGBA":
    ...
    return img.crop((0, 0, width, int(height * bust_fraction)))
...
def main():
    ...
    stats_lines = [
        ...
        "LinkedIn: linkedin.com/in/vinayak-bhatia-836556230",
        "Medium: medium.com/@ntpjc2vinayak",
    ]
